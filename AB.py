...
SOAms = 100 #Battelli, Agosta, Goodbourn, Holcombe mostly using 133
...
letterDurMs = 34 #23.6  in Martini E2 and E1b (actually he used 22.2 but that's because he had a crazy refresh rate of 90 Hz)
...
possibleCue1positions =  np.array([8,9,10]) # [4,10,16,22] used in Martini E2, group 2
possibleCue2lags = np.array([1,3,5,7,9])
    ...
    if numCasesInterframeLong >0 and (not demo):
       ...
       if longFramesStr != None:
                ...
                if not demo:
                    ...
                    #flankingAlso = flankingAlso[tnp.negative(np.isnan(flankingAlso))]  #remove nan values
                    flankingAlso = flankingAlso[~np.isnan(flankingAlso)]  #remove nan values
                    flankingAlso = flankingAlso.astype(np.int64) #cast as integers, so can use as subscripts
...
def handleAndScoreResponse(passThisTrial,responses,responsesAutopilot,task,letterSequence,cuesPos,correctAnswers):
    ...
    for i in range(len(cuesPos)): #print response stuff to dataFile
        ...
        correct = eachCorrect.all()  #if T1 correct & T2 correct, then correct=1; else, correct=0.
...
if doStaircase:
    ...
    while (not staircase.finished) and expStop==False: #staircase.thisTrialN < staircase.nTrials
        if staircaseTrialN+1 < len(prefaceStaircaseNoise): #still doing easyStaircaseStarterNoise
            ...
            if staircaseTrialN+1 == len(prefaceStaircaseNoise): #add these non-staircase trials so QUEST knows about them
                ...
                staircase.importData((100-prefaceStaircaseNoise).astype(np.int), np.array(corrEachTrial).astype(np.int))
    ...
    while nDoneMain < trials.nTotal and expStop==False:
        ...
        if not expStop:
            ...
            if task=="T1T2" and eachCorrect[0]==1:
            # if task=="T1T2":
            ...
            if (trials.nTotal > 6 and nDoneMain > 2 and nDoneMain %
                 round( trials.nTotal*pctCompletedBreak/100. ) ==1):  #dont modulus 0 because then will do it for last trial
...
if not doStaircase and (nDoneMain >0):
    ...
    if len(numTrialsEachCorrect) >1:
        ...
        print(nTrialsCorrectT2eachLag,nTrialsEachLag)
